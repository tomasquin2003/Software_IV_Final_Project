...
module Votacion {    // Enums para ServidorRegional (definidos primero)
    enum EstadoCache { ACTIVO, INACTIVO, SINCRONIZANDO, ERROR };
    enum TipoConsultaRegional { LUGARVOTACION, MESAASIGNADA, HORARIOS, DIRECCION };
    ...
    interface CacheRegional {
        ...
        DatosCache obtenerDatosCache(string cedula)
            throws CacheException;
        void actualizarCache(DatosCache datos)
            throws CacheException;        void limpiarCache()
            throws CacheException, ErrorPersistenciaException;
        EstadoCacheInfo verificarEstadoCache()
            throws CacheException;
    };
