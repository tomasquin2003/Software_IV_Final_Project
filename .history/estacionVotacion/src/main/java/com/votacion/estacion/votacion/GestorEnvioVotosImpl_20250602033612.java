...
public class GestorEnvioVotosImpl implements GestorEnvioVotos {
    ...
    @Override
    public String enviarVoto(String candidatoId, Current current) {        String votoId = UUID.randomUUID().toString();
        
        try {
            System.out.println("Enviando voto " + votoId + " para candidato: " + candidatoId);
            
            // Almacenar el voto localmente primero
            almacenamientoTransitorio.almacenarVotoTransitorio(votoId, candidatoId, EstadoVoto.PENDIENTE, new com.zeroc.Ice.Current());
